...
- **Tipografia**: IBM Plex Serif (títulos), IBM Plex Sans (corpo) e IBM
  ...
  Carregadas via Google Fonts (`<link>` no `<head>`).
...
- Testado visualmente nas telas de login, simular, empresas e num
  ...
  híbrido, limitações, análise de IA), nos dois temas — valores
...
**Quinta rodada — achatar em vez de decorar (mesmo dia).** Direção
**visual mais minimalista/premium** (referência: Linear/Stripe/Vercel),
não mais elementos. Isso inverteu o approach das rodadas 1-4, que foram
todas "adicionar" (cor, forma de componente, estrutura, cabeçalho de
página) — essa rodada é "tirar": menos sombra, menos gradiente, menos
cor decorativa, mais espaço em branco, tipografia carregando mais peso.
Só `base.html`:
...
Três saídas foram avaliadas: (A) campo de % de imposto embutido
